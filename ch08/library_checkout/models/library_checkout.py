...
class Checkout(models.Model):
    ...
    # ở dưới là code tự đặt tiêu đề cho form khi click vào vd library.member(3,)/2019-12-31
    @api.multi
    def name_get(self):
        names = []
        for rec in self:
            name = '%s/%s' % (rec.member_id, rec.request_date)
            names.append((rec.id, name))  # tên phải có rec.id
        return names
    ...
    # Thay đổi giá trịload member_id mà request không phải hôm nay sẽ báo nội dung
    # dưới - Chặn nhân viên thay đổi dữ liệu quá khứ member_id
